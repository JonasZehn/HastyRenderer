...
int LightRayInfo::sampleOrGetWavelength(RNG& rng, const Vec3f& sampleWeights, bool sample, float& sampleProbability)
{
  if(wavelength == -1 && sample)
  {
    Vec3f probabilities = sampleWeights / sum(sampleWeights);
    float xi = rng.uniform01f();
    if(xi < probabilities[0])
    {
      wavelength = 0;
      sampleProbability = probabilities[0];
    }
    else if(xi < probabilities[0] + probabilities[1])
    {
      wavelength = 1;
      sampleProbability = probabilities[1];
    }
    else
    {
      wavelength = 2;
      sampleProbability = probabilities[2];
    }
  }
  else
  {
